...
export default function Products() {
  ...
  // Keep state sync'd with hook data
  useEffect(() => {
    setLocalList(products);
  }, [products]);

  // Sync listener for offline storage updates
  useEffect(() => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      // 1. Delete from Firestore or localStorage
      await deleteProductRecord(deletingProduct);
      
      // 2. Clear images from Storage
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Saree Edit/Create Form Modal Overlay */}
      {showForm && (
        <ProductForm
      ...
      {/* Soft Delete Confirmation Modal prompt */}
      {deletingProduct && (
