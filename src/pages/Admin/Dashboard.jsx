...
import { Scissors, MessageSquare, AlertTriangle, UserCheck, ArrowRight } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (isPlaceholder) {
      ...
      let parsedProducts = localProducts;
      if (stored) {
        try { parsedProducts = JSON.parse(stored); } catch (e) {}
      }

      const oosCount = parsedProducts.filter(p => !p.availability).length;
    ...
    const unsubscribeInquiries = onSnapshot(qInq, (snapshot) => {
      ...
      setRecentInquiries(items.slice(0, 10));
      setLoading(false);
    }, (err) => {
      ...
      const oos = parsedProducts.filter(p => !p.availability).length;
      setStats({
        totalProducts: parsedProducts.length,
        totalInquiries: mockInquiries.length,
        outOfStock: oos,
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {statCards.map((card, idx) => {
          ...
          return (
            <div key={idx} className={`p-6 bg-white border border-neutral-200 rounded shadow-premium flex items-start gap-4 transition-all duration-300 hover:shadow-premium-hover`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white border border-neutral-200 shadow-premium rounded overflow-hidden">
          <div className="p-6 border-b border-neutral-200 flex justify-between items-center">
            ...
            <Link to="/admin/inquiries" className="text-xs text-primary font-semibold hover:underline flex items-center gap-1.5 uppercase tracking-wide">
